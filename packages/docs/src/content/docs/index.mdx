---
title: infra
description: Nix flake-based system configurations for NixOS, nix-darwin, and home-manager using three-layer architecture
template: splash
hero:
  tagline: Nix flake-based system configurations for NixOS, nix-darwin, and home-manager using three-layer architecture: flake-parts modular composition, nixos-unified directory-based autowiring, and multi-channel overlay composition.
  image:
...
Three-layer architecture combining complementary patterns:

1. **flake-parts** - Modular flake composition framework
2. **nixos-unified** - Directory-based autowiring (filesystem structure → flake outputs)
3. **Multi-channel overlay composition** - Per-package nixpkgs channel selection via overlays/infra/hotfixes.nix

Per-package channel selection enables selective stable fallbacks for broken packages without full flake.lock rollback affecting O(10^5) packages.
...
<CardGrid>
  <Card title="Understand the architecture" icon="open-book">
    Three-layer design combining flake-parts, nixos-unified, and multi-channel overlay composition.
  ...
  <Card title="Configure secrets" icon="lock">
    sops-nix integration with age encryption for declarative secrets management.
    [Secrets management guide →](/guides/secrets-management/)
  </Card>
  <Card title="Handle broken packages" icon="warning">
    Systematic workflow for per-package channel selection when nixpkgs unstable packages fail.
