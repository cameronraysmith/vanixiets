...
Run chainsaw integration tests against kubernetes/tests/local-k3d/ and
emit a coverage report categorizing deployed resources as application,
foundation, or system.

Options:
  --help   Show this message and exit 0
  --raw    Show raw uncategorized coverage output

Environment:
  CI / GITHUB_ACTIONS / NO_COLOR  Disable ANSI colors when set
EOF
    exit 0
    ;;
esac
...
main() {
    ...
    # Resolve test directory relative to the invoking worktree so the
    # flake app is location-independent.
    local repo_root
    repo_root=$(git rev-parse --show-toplevel)
    local test_dir="${repo_root}/kubernetes/tests/local-k3d"
    ...
    local test_failed=0
    if ! run_chainsaw_tests "$report_dir" "$test_dir" "${args[@]}"; then
        test_failed=1
    fi

    print_test_summary "$report_dir/chainsaw-report.xml"
    print_coverage_report "$test_dir"
