...
- Prefer performant CLI tools in shell commands: use `fd` instead of `find`, `rg` (ripgrep) instead of `grep`, `bat` instead of `cat` for display, `eza` instead of `ls`, and similar modern alternatives when available.
