      ...
      in
      {
        ...
        programs.tmux = {
          ...
          extraConfig = ''
            ...
            bind-key "i" display-popup -E -w 80% -h 80% "fd --type f --hidden --follow --exclude .git | fzf --preview 'bat --color=always --style=numbers --line-range=:500 {}' | cb copy"
            bind-key "I" display-popup -E -w 80% -h 80% "fd --type f --hidden --follow --exclude .git --absolute-path | fzf --preview 'bat --color=always --style=numbers --line-range=:500 {}' | sed 's|^${config.home.homeDirectory}/|~/|' | cb copy"
