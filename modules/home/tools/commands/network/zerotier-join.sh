...
# Network ID injected via NETWORK_ID env var (default: db4344343b14b903 from cinnabar controller)
set -euo pipefail

NETWORK_ID="${NETWORK_ID:-db4344343b14b903}"
...
if sudo zerotier-cli listnetworks 2>/dev/null | grep -q "${NETWORK_ID}"; then
  echo "Already joined zerotier network ${NETWORK_ID}"

  # Display current status
  NETWORK_STATUS=$(sudo zerotier-cli listnetworks | grep "${NETWORK_ID}" || true)
...
if [ -n "${MEMBER_ID}" ]; then
  ...
  NETWORK_PREFIX="fddb:4344:343b:14b9"  # Derived from network ID ${NETWORK_ID}
