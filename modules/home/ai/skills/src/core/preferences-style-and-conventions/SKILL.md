...
- Use performant CLI tools matched to task intent:
  ...
  - Notification (push alert to user): use `curl -k -d "<message>" https://ntfy.zt/<topic>` where `<topic>` is the machine hostname (from `hostname -s`, e.g. `stibnite`) and `<message>` includes the repo name and summarizes the completed task
