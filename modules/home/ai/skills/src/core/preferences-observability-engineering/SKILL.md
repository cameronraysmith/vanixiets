...
The "(or analog)" generalizes across three peer regulator kinds: process-compose is the lightest realization for application-composition envelopes, nspawn-based clan tests are the realization for service-on-NixOS envelopes, and full NixOS VM tests are the realization for NixOS-module-plus-kernel envelopes — the same observability contract can be asserted against any of the three depending on the artifact's deployment target, with escalation rules in `preferences-nix-checks-architecture` §"Choosing among integration regulators" and the application-composition end of the ladder detailed in `preferences-nix-checks-architecture/references/process-compose-checks.md`.
