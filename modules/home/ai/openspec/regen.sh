...
ver="$(nix eval --raw --impure --expr \
  "let f = builtins.getFlake (toString ${repo_root}); in f.inputs.llm-agents.packages.\${builtins.currentSystem}.openspec.version")"
...
(
  cd "${proj}" && HOME="${home}" CI=true OPENSPEC_TELEMETRY=0 \
    OPENSPEC_NO_COMPLETIONS=1 DO_NOT_TRACK=1 \
    bunx "@fission-ai/openspec@${ver}" init --tools claude --force </dev/null
)
