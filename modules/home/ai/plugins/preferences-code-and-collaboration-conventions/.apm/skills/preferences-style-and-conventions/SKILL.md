...
- Reference-repository lookups fall into two categories. *Category 1* is a repository we develop or maintain: it lives under `~/projects/<topic>-workspace/<repo>/` and follows the lookup-and-on-miss-clone steps immediately below. *Category 2* is a third-party dependency or research-reference repository we consult but do not maintain: acquire and review its upstream source through the ghq flow in the `dependency-source-acquisition` skill rather than the `~/projects/` clone convention. The steps below govern Category 1.
- When the user mentions a git repository by name (from GitHub or any other forge), or you are about to state a substantive technical claim grounded in a software project's source, options, defaults, API, or upstream documentation — first check for a local copy under `~/projects/` before reaching for web tools or asking the user for context. Treat the lookup as default for such claims; bypass it only with explicit justification. The steps:
