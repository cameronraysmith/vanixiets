...
It tightens the existing Scenario Coverage check inside openspec-verify-change, which already iterates every `#### Scenario:` marker and emits a non-blocking warning when a scenario appears uncovered, and which does not block archive.
