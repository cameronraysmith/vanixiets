...
**Subsequent qualification, not a rewrite of the original decision chain:** Q4 records fallback intent, not an unconditional delivered guarantee. GNOME is default only without saved choice; the September 9 niri choice took precedence (`verify.md` §9.1). A journal-proven `display-manager` restart **while niri was live** stranded niri and aborted authenticated GNOME startup; ordinary greeter switching is not an established trigger. The earlier decision 9.4 retained the unconditional promise as **UNSATISFIED**, correctly yielding historical **FAIL**. **Current 9.4 explicitly amends after that counterexample:** GNOME registered/selectable/default without recorded choice; the live-niri display-manager-restart state excluded. Only recovery WHEN narrows; THEN/AND retain working next-login GNOME, sign-out rather than repair and no advance preparation outside the exception. This is correction of a missing upstream lifecycle facility no configuration of ours can deliver — niri v26.04 lacks GNOME 50.1's leader-death bridge — not omission of an available configuration fix. [U-niri-fallback](known-limitations.md#verification-lesson-and-decided-undischarged-requirement) retains the incident, historical decision, rationale and **F-niri-upstream = [CAM-66](https://linear.app/cameronraysmith/issue/CAM-66/correct-password-returns-to-gdm-niri-survives-a-display-manager)** / open **9.3** for the excluded unrepaired case. Recovered GNOME login discharges 8.1, not the defect (§9.8). The independent amended verdict is §9.10; no monitor is restored.
