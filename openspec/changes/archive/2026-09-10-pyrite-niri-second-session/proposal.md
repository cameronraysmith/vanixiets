...
last_synced_at: 2026-09-10T03:36:31Z
review_round: 0
max_review_rounds: 3
attempt_log:
  ...
  - { at: "2026-09-10T03:35:44Z", transition: "archive->mirror", outcome: "posted", note: "Existing graphical-desktop-session document 8cf07dd5-2a48-4f45-bcd4-1add748aee46 updated in project 48b4123d589b; readback matches canonical content modulo Linear Markdown normalization. Workspace cameronraysmith and user Cameron Smith confirmed after both override variables were asserted unset." }
  - { at: "2026-09-10T03:35:44Z", transition: "proposal->description", outcome: "posted", note: "Business-facing issue description reconciled to final amendment and explicit coverage dispositions; no design/tasks mirror." }
  - { at: "2026-09-10T03:36:31Z", transition: "archive->Done", outcome: "posted", note: "Archive succeeded, local-date directory renamed to 2026-09-10-pyrite-niri-second-session, canonical mirror completed, issue Done read back; two-sentence comment 0b13f0b1-90c4-4a1b-af68-618f5874d4eb. Projection regenerated before move with 92 undischarged rows. Catch-up from live In Progress; no fabricated intermediate transitions." }
---
...
This change takes the first slice only: niri is a second login-screen entry; GNOME/GDM remain installed, GNOME registered/selectable at the greeter, default for users without saved choice and the designated fallback. **Amended after a demonstrated counterexample:** reachability does not hold when `display-manager` is restarted while niri is live; the requirement now explicitly excludes that state (task 9.4). Only recovery WHEN narrows; working next-login GNOME, sign-out rather than repair and no advance preparation remain promised outside that exception. This corrects an upstream lifecycle promise our configuration cannot deliver: niri v26.04 lacks GNOME 50.1's leader-death bridge, not an available configuration fix deliberately omitted. [U-niri-fallback](known-limitations.md#verification-lesson-and-decided-undischarged-requirement) preserves the counterexample and historical FAIL; **[CAM-66](https://linear.app/cameronraysmith/issue/CAM-66)** tracks the excluded, unrepaired case, not proof of upstream submission. **Final independent verdict: PASS WITH WARNINGS, archive-ready, not fully observed or formally discharged (`verify.md` §9.12).** 6.6 is runtime-discharged; **8.2 remembered choice is discharged but no-history GNOME is a source-read-only method limitation** because the operator declined destroying their real preference; use a disposable no-history user later. **8.3/8.4/8.5 evaluation, not observation; 8.6 declined CAM-59; 8.7 physical controls; whole-boot 1 requested / 1 entry.**
...
- To: the default session is pinned explicitly to the value that leaves saved sessions alone. Persisted niri choice is observed and the remembered-choice half is discharged; GNOME for a person with no recorded choice rests on GDM 50.1's fallback source and the explicit null/no-rewrite configuration, **not observation** (8.2 method limitation).
...
- To: the same guarantee holds for a niri session, established independently by evaluation under the operator's explicit 8.3/8.4 method decision, **not** by untouched mains/battery observation. Task 4.2 separately observed no idle consumers during a live niri session (`verify.md` §9.11).
...
**An ordinary power-button press in either niri or GNOME neither suspends nor powers off the host**
- From: niri hardcodes immediate suspend; disabling that alone would hand the key to logind's poweroff default. GNOME's independent media-keys handler also requested suspend, bypassing logind.
- To: disable niri power-key handling, make logind ignore the key, and set GNOME's `power-button-action='nothing'` (D9 and its explicit GNOME amendment).
- Reason: the only wake source must not also be an accidental route into unreliable sleep or poweroff. Deliberate suspension remains possible; its risky behavioral retest was declined under CAM-59.
- Impact: 8.7 passed by physical tests in both desktops with same-boot positive/negative controls; no change to greeter inactivity, lid or wake configuration. Whole-boot totals 1/1 are the pre-fix positive control, not zero.
...
**GNOME's and the login screen's existing inactivity configuration is preserved**
- From: `gdm.autoSuspend = false` and four inactivity dconf keys plus idle-delay.
- To: those existing values unchanged; GNOME's power-button-action is the sole explicit policy addition, so whole-database byte equality is not claimed.
- Reason: inactivity and ordinary key handling are independent mechanisms; the latter needed its own fix.
- Impact: no reorganization of the existing inactivity, greeter or lid policy.
