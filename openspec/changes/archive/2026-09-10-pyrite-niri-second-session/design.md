...
**Follow-up F-niri-upstream = [CAM-66 — Correct password returns to GDM: niri survives a display-manager restart](https://linear.app/cameronraysmith/issue/CAM-66/correct-password-returns-to-gdm-niri-survives-a-display-manager)**; [U-niri-fallback, historical failure and current narrow amendment](known-limitations.md#verification-lesson-and-decided-undischarged-requirement).
...
- GNOME under GDM remains registered/selectable at the greeter, the designated fallback and default for users with no saved choice, and per-user choice persists. **Amended after the counterexample:** reachability does not hold when display-manager is restarted while niri is live; CAM-66 tracks that excluded, unrepaired state. Task 9.4 supersedes the historical retain-unchanged decision because niri v26.04 lacks GNOME 50.1's leader-death bridge, not because an available configuration fix was omitted. Only the recovery scenario's WHEN is narrowed; its THEN/AND remain standing outside the exception. `verify.md` §9.10 independently assesses the amended requirement.
- A niri session does not suspend the host on inactivity, established independently by evaluation under the operator's 8.3/8.4 method decision, not untouched AC/battery observation; 4.2's separate runtime snapshot is now observed and discharged (`verify.md` §9.11).
...
- **A locally maintained niri lifecycle daemon.** The operator declined the proposed embedded C monitor as disproportionate to offering a second session, an indefinite maintenance burden in a machine module, and never end-to-end verified against real GDM/systemd on a machine with physical-only recovery. It is removed; the diagnosis/prototype survive only in upstream-report logs. `known-limitations.md` is the authoritative incident and operational record; tasks 9.1–9.4 track the disposition, not monitor deployment.
...
**Historical runtime clarification (2026-09-09): cameron's remembered session was niri.** GNOME is the default only for someone with no recorded choice; saved niri took precedence unless the cog selected GNOME. GDM 50.1 reads AccountsService (`daemon/gdm-session-settings.c:285-309`), prioritizes saved choice (`daemon/gdm-session.c:687-694`), and tries GNOME only in fallback (`:621-625`). Evidence: `verify.md` §9.1. **Updated 2026-09-10:** the recovered GNOME login is independently corroborated and task 8.1 discharged (§9.8); this is not a new AccountsService read. The live-niri display-manager-restart counterexample made the former unconditional fallback requirement **UNSATISFIED**. Task 9.4 now applies the explicit narrow amendment, not a repair; **F-niri-upstream = CAM-66** / 9.3 still tracks [U-niri-fallback](known-limitations.md#verification-lesson-and-decided-undischarged-requirement).
...
**gsd-power configuration versus runtime.** GNOME's power service is GNOME-target-gated and niri's delivered graph does not pull that target (`verify.md` §9.9). That configuration argument is now complemented by an actual complete affected-user process/service snapshot in niri (§9.11), not substituted for observation; greeter-user power processes are distinct from cameron's.

**Seen, not deduced — now discharged.** Source establishes no niri idle configuration, notifier/inhibitor rather than idle action, and effective logind `IdleAction=ignore`. Task 4.2 additionally observed no running idle consumer in active niri session 238, uid 1000/seat0/tty2, compositor PID 158454, at 2026-09-10 03:13:54 UTC (`logs/niri-archive-live-snapshot-20260909-231353.log`; `verify.md` §9.11). The corrected predicate permits benign autostarts: pinned v26.04 explicitly `Wants=xdg-desktop-autostart.target` (upstream `8ed0da44d974c32c6877d2f4630c314da0717ecb`, `resources/niri.service:8–9`), and that target is active. The old `xdg-autostart*` wildcard missed the target and generated `app-…@autostart.service` names. Full processes, running services and actual generated dependencies were inspected; no gsd-power, swayidle, hypridle, xss-lock, xidlehook or gsd-media-keys. This is a point-in-time live observation, not an untouched 35-minute idle test or a guarantee against future manually started consumers.

### D8: preserve GNOME/GDM inactivity protection; add only GNOME power-button protection

`gdm.autoSuspend = false`, the four inactivity dconf keys and `idle-delay = 1800` stay exactly as yesterday's change left them. The operator's GNOME amendment permits only `power-button-action = "nothing"` in the existing user database, without locks. This is not a power-policy unification or idle-policy rewrite; the whole database is no longer byte-identical, but its existing settings are. The verification is a scoped diff plus resolved evaluation (task 4.5).
...
Verification: tasks 2.5 and 4.4 check the built KDL and evaluated/rendered logind setting before deployment; task 4.5 evaluates GNOME's added dconf key. Task 8.7 observes an ordinary press doing neither suspend nor poweroff in **both fresh GNOME and fresh niri sessions**, after post-deploy gates 6.5/6.6. Greeter auto-suspend, lid and wake policy remain unchanged. Logind policy is host-wide, but does not govern a desktop's independent suspend request.
...
**GNOME amendment (2026-09-09; delivery corrected 2026-09-10).** The supplied 21:16:36 UTC suspend request came from `.gsd-media-keys` in GNOME, not niri. At gsd **50.1**, `plugins/media-keys/gsd-media-keys-manager.c:2142` reads the power-button enum, `:2174` dispatches, and `:3430-3444` takes the `handle-power-key` block inhibitor, bypassing logind policy. The added `power-button-action='nothing'` uses the schema's enum nick and returns without dispatch. This genuinely extends the former niri-only ordinary-press requirement to GNOME. **The fix IS deployed in generation 10 (00:11:53 UTC); running gsd-media-keys started 00:16:24 and directly maps the new database**, not merely an evaluated proposal (`verify.md` §9.8). For future database changes deploy alone does not reload an old daemon; a fresh GNOME session is needed, and any authorized display-manager restart requires niri inactive first. Task 6.6 retains only its remaining in-session observations; **8.7 is now physically discharged in both desktops with session-attributed same-boot controls** (§9.9). Exact-version source/eval/build evidence: `logs/niri-slice-a-powerbutton-gnome-final.md`; read-only delivery/adoption evidence: `logs/niri-cam66-final-runtime-20260910.log`, `logs/niri-cam66-final-daemon-20260910.log`. No deployed source commit is groundable.

**Final archive disposition (2026-09-10):** task 6.6 is now runtime-discharged under the operator's explicit disposition through live niri session 238 / compositor PID 158454's own environment: idle-delay `uint32 1800`, gsd power schema **absent from XDG_DATA_DIRS**, hence structurally inapplicable, and no block-mode `handle-power-key` inhibitor (`verify.md` §9.12). The original GNOME prerequisite transcript was not collected in its prescribed order. 8.2 is split: remembered choice discharged, no-history GNOME a **source-read-only method limitation**, not evaluation discharge; a disposable user can close it safely without destroying the operator's preference. Final verdict **PASS WITH WARNINGS, archive-ready**; 8.3/8.4/8.5 evaluation, 8.6 declined, 8.7 physical controls, whole-boot 1/1. CAM-66 remains the explicit unrepaired exception; Desktop= propagation remains unresolved.
...
- **A niri session is unusable at the panel.** → GNOME remains registered/selectable/default for users without saved choice; the amended recovery scenario retains working next-login GNOME, sign-out rather than repair and no advance preparation outside the explicit live-niri display-manager-restart exception. **Never restart display-manager with live niri: quit (`Mod+Shift+E`, confirm), then require cameron's `niri.service=inactive`; SSH/greeter location alone is insufficient.** The observed restart was the orchestrator's post-deploy sequence, not a stray action (`verify.md` §9.7). Ordinary greeter switching is not an established trigger. niri lacks GNOME's FIFO leader bridge; deliberate lingering/`moshi-hook` stays. Coordinated SSH recovery stops niri and the graphical target in the affected user's manager; side effects, exact-pin sources and evidence: `known-limitations.md`. Task 9.4 explicitly amends after the counterexample; **U-niri-fallback / F-niri-upstream = CAM-66**, open 9.3, remains unrepaired. Recovered GNOME usability closes 8.1 but does not erase this incident; current verdict is §9.10, no monitor restoration.
...
The session registration is additive: a second session appears while GNOME/GDM remain installed. Choosing GNOME at the login screen is the intended operational rollback **after clean niri exit**, not an unconditional delivered recovery path: the live-niri display-manager-restart incident requires the user-manager repair below. With `defaultSession = null` the configuration does not write AccountsService, though GDM persists the user's choice. Removing the change entirely removes its input and configuration, after which `sessionNames` returns to `["gnome"]` and the picker hides itself again; source removal alone does not clear already stranded runtime units.

Choosing GNOME also records GNOME for later logins. There is no configuration-driven AccountsService migration to unwind, but there is per-user choice state; neither that selection mechanism nor retention of GNOME establishes unconditional reachability.

**Incident qualification:** picking GNOME is the normal rollback only after a clean niri exit. If display-manager was restarted while niri was live, GNOME can abort until the stranded user units are stopped over SSH; see `known-limitations.md`. The operational no-live-niri restart rule applies to any future migration/redeployment. The source removal does not itself perform or authorize recovery, and does not make the unconditional fallback promise true.
