...
- [x] 4.1 `delivery=skills`: set `programs.openspec.delivery = "skills"`; rework the refresh sidecar (remove the command path, keep the exactly-11-skills assertion); remove the `assets/commands/` tree; remove the `commandsDir` wiring; update the affected doc-comments
- [x] 4.2 OpenSpec 1.5.0:
