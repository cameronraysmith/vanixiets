...
- **AND** `zpool labelclear -f <device>-part2` is retained as the first step rather than removed, because the block serves two mutually exclusive disk states and each of its first two steps is the live arm in one of them: the disk this change installs onto carries a live pre-D1 `zroot` with ZFS labels and no LUKS header at all, so `labelclear` is the live arm and `luksErase` reports that the device is not a valid LUKS device, while on any later reinstall p2 carries a LUKS2 container instead, so `luksErase` is the live arm and `labelclear` finds no label — the pool's labels then being inside the container and unreachable without opening it
