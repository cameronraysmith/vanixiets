...
> It last moved on 2026-07-17 and predates D1, so it is stale in three ways a reader would otherwise have to discover: it specifies ZFS native encryption with `aes-256-gcm` and a `postCreateHook` flipping `keylocation` to `prompt`, where the change now specifies a LUKS2 container with FIDO2 unlock and no dataset-level encryption at all; its Task 7 Step 2 prescribes `--phases disko`, which `tasks.md` 7.3 forbids outright because zeroing the kexec phase rewrites the ssh connection to `root@` and hangs forever on key upload after the wipe; and its step numbering does not correspond to the task numbering that has since been used everywhere else.
