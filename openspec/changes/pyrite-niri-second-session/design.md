...
- A niri session does not suspend the host on inactivity, established independently by evaluation under the operator's 8.3/8.4 method decision, not untouched AC/battery observation; 4.2's runtime observation remains open.
...
**gsd-power configuration versus runtime.** GNOME's power service is GNOME-target-gated and niri's delivered graph does not pull that target (`verify.md` §9.9). This supports the configuration argument, not an observed complete in-niri process snapshot; greeter-user power processes must be distinguished from cameron's.

**What was verified versus what was not.** Verified from source: niri has no idle configuration; niri implements only the notify and inhibit sides; logind's `IdleAction` default is `ignore` and pyrite's effective value is `ignore`. Not observed by this change: a complete in-niri process/service snapshot establishing that no package autostarts an idle consumer. **Correction:** the former “target is inert” inference was wrong: the delivered pinned niri v26.04 unit explicitly `Wants=xdg-desktop-autostart.target` (`logs/niri-final-verdict-dconf-autostart-20260910.log:38–53`; exact upstream `8ed0da44d974c32c6877d2f4630c314da0717ecb`, `resources/niri.service:8–9`). Benign autostarts run; the old task's `xdg-autostart*` wildcard misses both that target and generated `app-…@autostart.service` names. Task 4.2 must establish **no idle consumer**, not no autostart, by an actual in-niri observation — **seen, not deduced**. Correcting the predicate preserves this evidence method: observation only, no deploy needed; 4.2 stays unchecked.
