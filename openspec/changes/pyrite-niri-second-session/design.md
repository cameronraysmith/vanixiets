...
**GNOME amendment (2026-09-09).** The supplied 21:16:36 UTC suspend request came from `.gsd-media-keys` in GNOME, not niri. At gsd **50.1**, `plugins/media-keys/gsd-media-keys-manager.c:2142` reads the power-button enum, `:2174` dispatches the selected action, and `:3430-3444` takes a `handle-power-key` block inhibitor: logind's `HandlePowerKey` is bypassed. Add `org.gnome.settings-daemon.plugins.power power-button-action='nothing'`; the schema explicitly names this enum nick, and its action branch returns without dispatch. This extends the ordinary-press requirement to GNOME rather than claiming the earlier newly-offered-desktop clause already covered it. Restart the GNOME session after redeployment (logout/login or authorized display-manager restart); the store-backed dconf source is not reloaded into the existing daemon by deploy alone. Task 6.6 is the third reload gate after 6.4 and 6.5. Exact-revision citations, source semantics and actual eval/build output: `logs/niri-slice-a-powerbutton-gnome-final.md`. The prior GNOME press tested nothing about the proposed protection; both fresh-session checks remain open.
