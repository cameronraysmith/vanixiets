...
The decided path is to abandon the cognee MCP entirely (client and server), expose the REST API directly over the mesh bound ZeroTier-only, point the plugin and a global cognee-cli at it through one nix source of truth, and add a kanidm-gated browser UI by registering cognee as the first consumer of the shared `sso-gateway` (delivered by its own change), which leaves buildbot's existing oauth2-proxy singleton fully untouched.
...
It establishes a single typed nix source of truth, drops the cognee MCP on both sides, exposes the REST API bound ZeroTier-only behind per-agent authentication, points the always-on plugin and a new global cognee-cli wrapper at central magnetite with per-host namespacing, and adds a kanidm-gated public UI at `kb.scientistexperience.net` by registering cognee as consumer #1 of the shared `sso-gateway`.
There is no Phase-1/Phase-2 split and no buildbot prerequisite change.
This change depends on the `sso-gateway` change, which delivers the reusable shared kanidm-OIDC perimeter (the `oauth2-proxy-kanidm` unit, the `sso-gateway` kanidm client, the `auth.scientistexperience.net` subdomain, and the cookie/client-secret generators); cognee owns no oauth2-proxy, no kanidm client, and no perimeter secret of its own.
The one cross-repo prerequisite is rebuilding the cognee frontend in the cognee-nix fork (a `configurePhase` `substituteInPlace` of the backend-URL fallback plus a flake-input bump); multi-tenant per-dataset isolation, multi-user public access, cognee native OIDC, and a public machine path remain deferred future work.
...
**Public kanidm-gated browser UI via the shared sso-gateway**
- To: rebuild the cognee frontend in the cognee-nix fork by patching the backend-URL fallback `|| "http://localhost:8000"` per file class (`""` for the 6 client/shared files so the browser calls same-origin `/api/`, an absolute loopback URL for the 2 Node route handlers, the public FQDN for the 2 copyable-URL display components), run it on loopback `127.0.0.1:3000`, and register cognee as consumer #1 of the shared gateway via `sso.services.cognee = { domain = "kb.scientistexperience.net"; allowedGroups = [ "cognee_access" ]; upstream = { "/" = loopback frontend; "/api/" = ZeroTier REST; }; }`; the gateway emits the `kb` vhost (routing `/` to the loopback frontend with a browser sign-in redirect and `/api/` to the ZeroTier REST with `error_page 401 =401;` fail-fast), owns the `oauth2-proxy-kanidm` unit and the `sso-gateway` kanidm client, and auto-derives the `cognee_access` group stub plus the client's `scopeMaps`/`claimMaps.groups` from cognee's registration.
- Reason: cognee has no native OIDC, so a kanidm perimeter gates the browser path; buildbot already owns the host oauth2-proxy singleton, so the gate needs a second oauth2-proxy instance — and the shared `sso-gateway` is exactly that, reused across services rather than re-stood-up per service; with `REQUIRE_AUTHENTICATION` on, the perimeter and app-auth are defense-in-depth.
- Impact: cognee adds one `sso.services.cognee` registration and one new `kb` DNS record; the `oauth2-proxy-kanidm` unit, the `sso-gateway` kanidm client, the `cognee_access` group derivation, the cookie/client-secret generators, and the `auth.scientistexperience.net` record are all owned by the `sso-gateway` change. Buildbot is left entirely untouched.

## Capabilities

### New Capabilities
- `cognee-knowledge-base`: a single canonical nix source of truth for the cognee endpoint consumed by the cognee server bind, the always-on plugin env, a global cognee-cli wrapper, the public-UI FQDN, and the terranix DNS record; the cognee MCP dropped on both sides with its orphaned systemd stanzas removed; the REST API bound ZeroTier-only behind `REQUIRE_AUTHENTICATION` with a build-time no-public-bind assertion; a scoped per-host `X-Api-Key` consumed by the plugin and the CLI; the plugin always-on with per-host dataset/agent namespacing and auto-union recall; a kanidm-gated public browser UI delivered by registering cognee as consumer #1 of the shared `sso-gateway` (`sso.services.cognee`) — the gateway owning the oauth2-proxy, the kanidm client, and the perimeter secrets — with the same-origin frontend rebuilt via a per-file-class fallback patch; multi-tenant per-dataset isolation deferred as a one-way door; and buildbot left untouched.
