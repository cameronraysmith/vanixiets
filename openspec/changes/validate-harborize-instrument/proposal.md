...
- New host prerequisites, currently unmet: a running Docker daemon (OrbStack is stopped and the socket is absent), plus `harbor` and `bench` installed outside the read-only ghq reference clones through `uv tool install` or a scratch `UV_PROJECT_ENVIRONMENT`.
...
- Metered API spend on the final rung only, at metered rates, with `ANTHROPIC_OAUTH_TOKEN` scrubbed on any Pi cell and `HARBOR_TELEMETRY=0` on every Harbor line.
