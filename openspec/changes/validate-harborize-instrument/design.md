...
- **Free consequence exploited**: claude-code's destination sits inside the `/logs/agent` bind mount (`trial.py:1284-1288`), so what the adapter registered is readable on the host after the run with no verifier code at all. That is the cheapest per-adapter assertion available and it is why claude-code is the first metered cell.
- **Scope of the Harbor-arm assertion**: it is the destination directory alone and deliberately not the reward, which follows from D11's shared fork. A real agent can reach the token through the verifier without the skill being delivered, so the metered trial's reward carries no injection information in either direction and is not read as evidence.
...
- **Choice**: each ladder rung is witnessed by the runner's own artifact — a docker exit status, a python-level refusal, `bench tasks check`'s issue list, a reward file, or a directory listing inside the `/logs/agent` bind mount — recorded in tasks.md and verify.md. No `.feature` files and no step-definition runner are added.
...
### D11: the canary runs a shared verifier, and its oracle-to-verifier channel is `/logs/verifier`

- **Choice**: the injection canary declares no verifier sandbox table and ships no `verifier/Dockerfile`, so it runs shared; its oracle writes `/logs/verifier/canary-output.txt` and its verifier reads the same path. The mechanical package keeps the fork as a recorded per-package decision and is where separate mode gets exercised.
- **Rationale**: the fork is forced rather than preferred. Under BenchFlow a separate verifier sandbox is "parsed but not executed" (`src/benchflow/task/runtime_capabilities.py:186-192`), so the verifier runs in the agent sandbox whatever the package declares; under Harbor a separate verifier container receives exactly one bind, `self.paths.verifier_dir` to `/logs/verifier`, and no agent-workspace mount (`src/harbor/trial/trial.py:681-692`). A canary that wrote to a workspace path such as `/app` would therefore pass under one runner and fail under the other, which is the most confusing failure available to a change whose purpose is removing that confound.
- **Why `/logs/verifier` rather than the workspace**: that directory is bind-mounted into the agent environment (`trial.py:1279-1283`) and into a separate verifier environment (`trial.py:686-692`) from the same host directory, so the channel survives a later change of the fork. Harbor does not clear it before reading the reward (`verifier/verifier.py:199-236`), and it is BenchFlow's standard verifier contract path.
- **Cost accepted**: everything in the shared verifier is visible to the agent under test. That is free here and would not be on a capability measurement — the canary measures no capability, its answer key is already in the injected skill by construction, and neither pass criterion that depends on it reads a reward from a model-driven trial.
...
| Adapter registration is asserted per adapter | Dependency compatibility / import smoke | `smoke` | the adapter's destination directory carries the skill, read from the `/logs/agent` bind mount |
