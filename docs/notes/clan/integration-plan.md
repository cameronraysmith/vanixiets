...
+The migration follows a validation-first approach with infrastructure deployment: validate dendritic + clan integration in test-clan/ (Phase 0), deploy real infrastructure to Hetzner Cloud and GCP via terraform/terranix from test-clan while proving the complete stack (Phase 0), then migrate nix-config to production integration patterns and migrate darwin hosts progressively (blackphos → rosegold → argentum → stibnite) in Phases 1-5.
+Phase 0 de-risks the migration by proving the architectural combination and complete infrastructure stack (dendritic + clan + terraform + cloud providers) work together before touching production nix-config.
+The approach validates dendritic + clan on NixOS first, deploys always-on infrastructure from test-clan, then transitions to production nix-config integration while eliminating nixos-unified and adopting clan-core's inventory system, vars management, and multi-machine service coordination using the dendritic flake-parts pattern's `flake.modules.*` namespace.
...
+- GCP infrastructure (TBD): Additional cloud resources deployed via terraform
...
+0. **test-clan validation + infrastructure**: Validate dendritic + clan in test-clan/, deploy cinnabar VPS and GCP infrastructure from test-clan, prove complete stack before production
+1. **nix-config production integration**: Apply proven patterns to nix-config, migrate blackphos as first darwin host
+2. **rosegold**: Validates darwin patterns are reusable, multi-machine coordination
+3. **argentum**: Final validation before primary workstation
+4. **stibnite**: Primary workstation, migrate only after all others proven stable
+5. **cleanup**: Remove nixos-unified, finalize migration
...
+│   │   ├── cinnabar/              # Phase 0: VPS infrastructure (deployed from test-clan)
 │   │   │   ├── default.nix
 │   │   │   ├── disko.nix
...
+│   │   ├── blackphos/default.nix  # Phase 1: First darwin (production nix-config)
+│   │   ├── rosegold/default.nix   # Phase 2: Second darwin
+│   │   ├── argentum/default.nix   # Phase 3: Third darwin
+│   │   └── stibnite/default.nix   # Phase 4: Primary workstation
...
+Phase 0: Pattern validation + Infrastructure deployment (test-clan/)
...
+- Validate dendritic + clan integration
+- Setup terraform/terranix for Hetzner Cloud and GCP
+- Deploy cinnabar VPS and GCP infrastructure from test-clan
+- Validate complete stack (dendritic + clan + terraform + cloud)
...
+Phase 1: Production integration (nix-config + blackphos)
+- Apply proven patterns from test-clan to nix-config
+- Add clan-core, import-tree inputs to nix-config
+- Create modules/ structure with validated patterns
+- Transfer cinnabar management to nix-config or keep in test-clan
+- Migrate blackphos as first darwin host
+- Validate darwin + clan integration in production
 
-Phase 3: Second darwin host (rosegold)
+Phase 2: Second darwin host (rosegold)
...
+**Risk without Phase 0**: Deploying untested architectural combination directly to production nix-config creates compound debugging complexity across 8 simultaneous layers (dendritic, clan, terraform, hetzner, disko, LUKS, zerotier, NixOS).
 
-**Solution: Phase 0 validation in test-clan/**
+**Solution: Phase 0 validation + deployment in test-clan/**
...
+- Deploy real infrastructure (Hetzner Cloud VPS + GCP) from test-clan
+- Test complete stack: dendritic + clan + terraform + cloud providers
+- Test all critical integration points before production nix-config commitment
 - Document findings and extract patterns
-- Prove architectural combination works
-- Reduce risk before VPS deployment
+- Prove architectural combination AND operational deployment work together
+- Reduce risk before production nix-config migration
 
-**Expected outcome**: Proven patterns ready for cinnabar deployment with confidence.
+**Expected outcome**: Proven patterns AND operational infrastructure ready for production nix-config integration with confidence.
...
+**Objective**: Validate dendritic + clan integration AND deploy real infrastructure to prove complete stack
 
 **Foundation**: clan + flake-parts (proven in clan-core, clan-infra)
 **Experiment**: How much dendritic optimization is compatible?
+**Deployment**: Real Hetzner Cloud VPS (cinnabar) + GCP infrastructure
...
+  - Add terranix, disko, srvos flake inputs
+  - Setup terraform/terranix for Hetzner Cloud and GCP
+  - Initialize clan secrets (age keys, Hetzner API tokens, GCP credentials)
+  - Create cinnabar host configuration with disko for LUKS encryption
+  - Configure zerotier controller role on cinnabar
+  - Deploy VPS and GCP resources via terraform + clan machines install
+  - Validate complete infrastructure stack
 
-**Outcomes** (all valid):
+**Outcomes** (all valid for patterns):
...
+- [ ] Hetzner Cloud VPS (cinnabar) deployed and operational
...
+**Not a pass/fail on full dendritic adoption.**
+**Is a characterization of what's feasible and beneficial PLUS operational infrastructure deployment.**
+
+**Timeline**: 1-2 weeks for implementation + validation + deployment, 1-2 weeks monitoring stability
+
+**Detailed guide**: `01-phase-0-validation.md`
+
+**Optional but strongly recommended**: Phase 0.5 (darwin validation) - see Phase 0 guide for details. Validates darwin + clan + dendritic integration in a test environment before production darwin deployment (Phase 1).
+
+### Phase 1: Production integration (nix-config + blackphos)
...
+**Strategic value**: Brings validated dendritic + clan patterns and operational infrastructure into production configuration management
+
 **Tasks**:
...
+  - Transfer or redeploy cinnabar management to nix-config (or keep in test-clan as infrastructure repo)
 ...
 **Success criteria**:
+- [ ] nix-config flake evaluates with clan-core and dendritic patterns
...
+**Timeline**: 1-2 weeks for integration + 1-2 weeks monitoring stability
+
+**Detailed guide**: `02-phase-1-production-integration.md`
+
+**CRITICAL**: Phase 0 deploys infrastructure from test-clan. Phase 1 migrates production nix-config to use validated patterns.
+
+### Phase 2: Second darwin host (rosegold)
+
+**Repository**: `~/projects/nix-workspace/nix-config/` (production config, `clan` branch)
+
+**Objective**: Validate darwin patterns are reusable, test multi-machine coordination
...
-### Phase 4: Third darwin host (argentum)
+**Detailed guide**: `03-phase-2-rosegold-guide.md`
+
+### Phase 3: Third darwin host (argentum)
+
+**Repository**: `~/projects/nix-workspace/nix-config/` (production config, `clan` branch)
...
-### Phase 5: Primary workstation (stibnite)
+**Detailed guide**: `04-phase-3-argentum-guide.md`
+
+### Phase 4: Primary workstation (stibnite)
+
+**Repository**: `~/projects/nix-workspace/nix-config/` (production config, `clan` branch)
...
+**Detailed guide**: `05-phase-4-stibnite-guide.md`
+
+### Phase 5: Cleanup
+
+**Repository**: `~/projects/nix-workspace/nix-config/` (production config, `clan` branch)
+
+**Objective**: Remove legacy infrastructure, finalize migration
...
+- Final validation of clean architecture
 
 **Success criteria**:
 - [ ] Clean dendritic + clan architecture
 - [ ] No nixos-unified remnants
 - [ ] Documentation updated
+- [ ] All hosts operational with new architecture
+
+**Detailed guide**: `06-phase-5-cleanup-guide.md`
...
+**Chosen**: test-clan (validation + infrastructure) → nix-config (production integration + blackphos) → rosegold → argentum → stibnite → cleanup
 **Rationale**:
...
+- **Test-clan first with infrastructure deployment**: Validates integration AND deploys infrastructure before production commitment
+- **Infrastructure in Phase 0**: Deploys Hetzner Cloud VPS and GCP infrastructure from test-clan, proving complete stack
+- **Production nix-config in Phase 1**: Applies validated patterns to production, migrates blackphos
+- **Always-on infrastructure from test-clan**: Provides stable zerotier controller independent of darwin hosts
+- **De-risks production migration**: Complete stack (dendritic + clan + terraform + cloud) proven working before nix-config changes
+- **Progressive darwin validation**: Each host validates patterns before moving to next
...
+**Tradeoff**: Adds Hetzner Cloud cost (~€24/month) and 2-4 weeks validation time (Phase 0), but provides significant risk reduction by proving everything works before production changes
...
+1. **Immediate**: Read `01-phase-0-validation.md` for dendritic + clan integration validation and infrastructure deployment
+2. **Phase 0**: Validate integration in test-clan/, deploy cinnabar VPS and GCP infrastructure, document findings
...
+4. **Phase 1**: Apply proven patterns to production nix-config, migrate blackphos (read `02-phase-1-production-integration.md`)
+5. **Phase 2**: Migrate rosegold (read `03-phase-2-rosegold-guide.md`), validate darwin patterns are reusable
+6. **Phase 3**: Migrate argentum (read `04-phase-3-argentum-guide.md`), final validation before primary workstation
+7. **Phase 4**: Migrate stibnite only after all others proven (read `05-phase-4-stibnite-guide.md`)
+8. **Phase 5**: Clean up legacy infrastructure (read `06-phase-5-cleanup-guide.md`)
