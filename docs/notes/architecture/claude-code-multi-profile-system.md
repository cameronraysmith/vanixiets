# Claude Code multi-profile system

## Overview

Declarative, profile-based configuration system for Claude Code CLI that enables seamless switching between multiple API providers (Anthropic official, GLM-4.6 via Z.ai) while maintaining complete isolation and reproducibility.
...
The system follows a **wrapper-based approach** that:
- Creates standalone wrapper modules without modifying existing configuration
- Leverages `CLAUDE_CONFIG_DIR` environment variable for profile isolation
- Shares configuration via nix store symlinks (single source of truth)
- Manages secrets declaratively with sops-nix integration
...
This multi-profile system provides:
