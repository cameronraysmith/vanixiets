...
export async function runSandboxChecks({ tools, slices }) {
  ...
  for (const slice of slices) for (const gate of slice.gates) {
    ...
    const sharedRuntime = [10, 11].includes(slice.id) && gate.kind === "Command" && gate.argv[1]?.endsWith("/runtime-contract-checks.py");
    ...
    if (gate.kind === "Command" && gate.argv[0] === "python3") {
      if (slice.id === 11 && !sharedRuntime) {
        assert(command.startsWith("'python3' '/primary/.atomic/workflows/omnigent/tooling-resolution.py' '--binding-expr'"));
        assert(command.includes(source.source) && command.includes("'--host'"));
        continue;
      }
      const expected = sharedRuntime
        ? "'python3' '/primary/.atomic/workflows/omnigent/runtime-contract-checks.py' '/sandbox'" + (slice.id === 11 ? " '--runtime-tooling'" : "")
        : slice.id === 7
