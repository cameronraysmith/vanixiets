...
const files = [entry, ...["types", "tools", "slices", "prompts", "deployment", "s9-s10", "s11"].map((name) => `.atomic/workflows/omnigent/${name}.ts`)];
const options = {
...
const tooling = await import(moduleUrl(".atomic/workflows/omnigent/s11.ts"));
if (process.argv.includes("--s11-empirical")) {
  await (await import("./s11-checks.mjs")).runToolingEmpirical({ contract, tooling });
  process.exit(0);
}
await (await import("./s11-checks.mjs")).runToolingChecks({ contract, additions, tooling });
...
if (process.argv.includes("--identity-audit")) {
  (await import("./identity-checks.mjs")).runIdentityChecks(contract, { audit: true });
  process.exit(0);
}
...
execFileSync("python3", [".atomic/workflows/omnigent/tooling-checks.py"], { stdio: "inherit", env: { ...process.env, PYTHONDONTWRITEBYTECODE: "1" } });
...
assert.equal(types.inputs.start_at_slice.maximum, 12);
assert.equal(types.Slice.properties.id.maximum, 11);
...
assert(main.includes("changes[3]!.change_id") && main.includes("changes[2]!.change_id") && main.includes("current = 12"));
