...
import { toolingSlice } from "./s11.js";
...
]), toolingSlice(plan)] as const satisfies readonly Slice[];
...
export const joinGateSupersessions: readonly GateSupersession[] = [
  {
    superseded: gateReference(1, 0, "Linux `packages.x86_64-linux.omnigent.version` equals 0.12.0"),
    superseding: gateReference(9, 0, "Linux `packages.x86_64-linux.omnigent.version` equals 0.13.0"),
  ...
  ...([
    [0, "Server `c.services.omnigent.enable` is true"],
    [1, "Server `c.services.omnigent.domain` equals omni.scientistexperience.net"],
    [2, "Cookie-secret `c.services.omnigent.cookieSecretGenerator` equals omnigent-cookie-secret-omnigent"],
    [3, "Omnigent, Matrix and Buildbot retained in `c.services.postgresql.ensureDatabases`"],
    [4, "PostgreSQL `c.services.postgresql.package.version` has prefix 16."],
    [5, "Server `c.systemd.services.omnigent.after` includes `postgresql.target`"],
    [6, "Server uses `postgresql+psycopg:///omnigent?host=/run/postgresql`"],
    [7, "PostgreSQL peer authentication retained in `c.services.postgresql.authentication`"],
    [8, "nginx `.proxyWebsockets` enabled"],
    [9, "nginx `proxy_read_timeout 1d;`, `proxy_send_timeout 1d;` and `proxy_buffering off;`"],
    [12, "Both Omnigent secret generators checked with `builtins.hasAttr n c.clan.core.vars.generators`"],
    [13, "Server `c.systemd.services.omnigent.environment.WEB_CONCURRENCY` equals one"],
    [14, "Server `c.systemd.services.omnigent.requires` includes `postgresql.target`"],
    [15, "Server `.serviceConfig.MemoryHigh` and `.serviceConfig.MemoryMax` equal 2G/3G"],
    [16, "Neither `OMNIGENT_OIDC_ALLOWED_DOMAINS` nor `OMNIGENT_OIDC_SKIP_EMAIL_VERIFICATION` exists"],
    [17, "Rendered `OMNIGENT_ADMIN_LIST_PATH` equals the primary email and newline"],
    [18, "Server binds loopback with `--host 127.0.0.1`"],
    [19, "nginx `.enableACME` and `.forceSSL` enabled"],
    [20, "nginx `.proxyPass` uses `http://127.0.0.1:`"],
  ...
  {
    superseded: gateReference(2, 25, "Synthetic server fixture `system.build.toplevel.drvPath` evaluates to a store derivation path"),
    superseding: gateReference(3, 37, "Real inventory `checks.x86_64-linux.nixos-magnetite` toplevel derivation evaluates and builds remotely"),
  ...
  {
    superseded: gateReference(3, 34, "Host roster `builtins.attrNames` of `).clan.inventory.instances.omnigent.roles.host.machines` equals magnetite alone"),
    superseding: gateReference(8, 0, "Host roster `builtins.attrNames` of `).clan.inventory.instances.omnigent.roles.host.machines` equals magnetite, pyrite, stibnite"),
  ...
  {
    superseded: gateReference(4, 0, "Rendered server `OMNIGENT_CONFIG_HOME` config has exactly one Atomic row counted by `.acp.agents | length`"),
    superseding: gateReference(6, 2, "Rendered server `OMNIGENT_CONFIG_HOME` config compares `.acp.agents` with `jq -e --argjson expected` for exactly Atomic and Oh My Pi"),
  ...
  {
    superseded: gateReference(4, 1, "Runner `builtins.length s.acp.agents` and declared `s.host.name` equal one and magnetite"),
    superseding: gateReference(10, 3, "Magnetite runtime regression: `h.programs.omnigent.settings.host.name == \"magnetite\"` and exact Atomic/Oh My Pi array via `h.programs.omnigent.settings.acp.agents == builtins.fromJSON`"),
  ...
  {
    superseded: gateReference(4, 2, "Magnetite `nixosConfigurations.magnetite.config.home-manager.users.cameron.home.packages` contains `omnigent-0.12.0`"),
    superseding: gateReference(9, 22, "Both Linux `nixosConfigurations.magnetite` and standalone Darwin `homeConfigurations.\"crs58@aarch64-darwin\"` homes contain `omnigent-0.13.0`"),
  ...
  {
    superseded: gateReference(4, 3, "Standalone `homeConfigurations.\"crs58@aarch64-darwin\"` home contains `omnigent-0.12.0`"),
    superseding: gateReference(9, 22, "Both Linux `nixosConfigurations.magnetite` and standalone Darwin `homeConfigurations.\"crs58@aarch64-darwin\"` homes contain `omnigent-0.13.0`"),
  ...
  {
    superseded: gateReference(4, 4, "Runner `merge-config.sh` preserves `.host.host_id` and `.host.name` with exactly one Atomic row counted by `.acp.agents | length`"),
    superseding: gateReference(6, 3, "Runner `merge-config.sh` preserves `.host.host_id` and `.host.name` with `jq -e --argjson expected` for exactly Atomic and Oh My Pi"),
  ...
  },
  {
    ...
    superseding: gateReference(9, 4, "Isolated Linux `packages.x86_64-linux.omnigent.outPath` console interpreter runs `env -i $py -I -` with `package-probe.py`: imports and invokes omnigent.harnesses.claude_native.hook; old module absent"),
  ...
  {
    superseded: gateReference(7, 0, "Host roster `builtins.attrNames` of `).clan.inventory.instances.omnigent.roles.host.machines` equals magnetite and stibnite"),
    superseding: gateReference(8, 0, "Host roster `builtins.attrNames` of `).clan.inventory.instances.omnigent.roles.host.machines` equals magnetite, pyrite, stibnite"),
    property: "Exact host-role inventory membership, including absence of extra hosts; both compare the same attrNames expression.",
  ...
  ...([[7, 10], [8, 22]] as const).map(([slice, index]): GateSupersession => ({
    superseded: gateReference(slice, index, "Darwin empty-extras PATH `a.EnvironmentVariables.PATH == expected` equals the eleven original harness/runtime bins plus system suffix; `builtins.head a.ProgramArguments` equals configured package"),
    superseding: gateReference(11, 0, "Darwin `g.darwinConfigurations.stibnite` empty-extras `path == expected` includes `p.bash p.which p.direnv p.nix p.gh`; `builtins.head h.launchd.agents.omnigent-host.config.ProgramArguments` equals configured package"),
    property: "Exact ordered stibnite PATH, empty extras, system suffix and configured package executable identity. S11 retains all S10:8 conjuncts and adds gh; this directly active carrier also preserves the original S7/S8 predicates.",
    falseAtJoin: "S10 added four runtime bins and S11 adds gh, contradicting the historical eleven-bin equality. Candidate witness based on join 32b75b3b1e34aed7f411d77d19a97792d5fae245 plus S11 files: both old NixEval predicates return false and S11:0 returns true (exit 0), logs/omnigent-s11-empirical3-20260910.log. This is not a routed-join witness; deployment must recheck after routing.",
  })),
  {
    superseded: gateReference(10, 8, "Darwin `g.darwinConfigurations.stibnite` exact `path == expected` and `builtins.head h.launchd.agents.omnigent-host.config.ProgramArguments`"),
    superseding: gateReference(11, 0, "Darwin `g.darwinConfigurations.stibnite` exact `path == expected` includes `p.nix p.gh` and `builtins.head h.launchd.agents.omnigent-host.config.ProgramArguments`"),
    property: "Full ordered PATH, empty extraPackages, system suffix and configured-executable equality; unchanged conjuncts with gh appended to the reviewed runtime list.",
    falseAtJoin: "S11 adds gh before the system suffix, so S10's exact PATH equality is false. Candidate witness based on join 32b75b3b1e34aed7f411d77d19a97792d5fae245 plus S11 files: S10:8 returns false and S11:0 returns true (exit 0), logs/omnigent-s11-empirical3-20260910.log. Routed-join evidence remains required after routing.",
  },
  {
    superseded: gateReference(10, 18, "Shared runtime structural check `runtime-contract-checks.py`"),
    superseding: gateReference(11, 1, "Shared runtime structural check `runtime-contract-checks.py` with `--runtime-tooling`"),
    property: "Exact common/Linux package sets, both shared consumers before extras, platform partition and consumer account-literal checks; the opt-in S11 expectation adds only gh/procps/lsof and leaves historical defaults unchanged.",
    falseAtJoin: "S11 adds gh to the common set and procps/lsof to the Linux set, so S10's exact structural expectations are false. On the candidate based on join 32b75b3b1e34aed7f411d77d19a97792d5fae245 plus S11 files, old checker exits 1 and S11 checker exits 0 (logs/omnigent-s11-empirical3-20260910.log). gh-only and cleanup-only controls also reject; routed-join evidence remains required after routing.",
  },
];
