...
export default workflow({
  ...
  run: async (ctx) => {
    ...
    try {
      ...
      if (!alreadyEnabled) {
        const previousPhase = host === "magnetite" ? "inventory" : host === "pyrite" ? "magnetite" : "pyrite";
      ...
      const enrollment = await ctx.ui.editor(`Enroll ${host} workers — non-secret record only. Accounts: ${JSON.stringify(metadata)}\nFor each actual worker HOME, use umask 077 and independent logins; never copy a complete human directory or credential store.\nOmnigent: omnigent login https://omni.scientistexperience.net (human opens browser URL and verifies owner).\nGitHub: gh auth login --hostname github.com --git-protocol https --web --insecure-storage; confirm broad personal grant deliberately.\nLinear CLI: linear auth login --plaintext with the human's API key entered privately; if using MCP OAuth instead, stop for a supported probe/contract adjustment, not a bot substitution.\nModels: each selected harness's installed-version login; do not share copied refresh grants.\nRecord actual owner, GitHub login, Linear identity/workspace, host ID and selected harnesses for each person, with no credentials. Existing enrollment can be verified rather than repeated.\n`);
      if (!enrollment?.trim()) throw new Stop("human", "Enrollment record missing");
      const enrollmentPath = await ops.save("enrollment", { host, kind: "human_attestation", record: enrollment });
      if (!await ctx.ui.confirm("Have all listed humans personally confirmed their host owner, GitHub/Linear identities and selected model entitlement under these worker accounts? Credentials must stay private; confirm provider authority is intentional. No existing sessions are automatically rerouted.")) throw new Stop("human", "Enrollment incomplete");
