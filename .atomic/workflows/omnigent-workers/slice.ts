...
import { childInputs, outputs, slice, invariants, research, repository, StageReport, Review, Stop, rethrowControl, parse, supplementalProtection, type Source, type Phase } from "./contract.js";
import { Operations, digest, integrated, foreignContextChanged } from "./operations.js";
import { gates, baselineExpr, compareProtected, type Protected } from "./gates.js";

type Projection = { phase: Phase; source: Source; path: string; digest: string; expression: string; expressionDigest: string; controller: string; command: string };
export async function projection(ops: Operations, name: string, source: Source, phase: Phase = "inventory"): Promise<Projection> {
  return ops.tool(name, { source, phase }, async (signal) => {
    const expression = `${ops.root}/${name}.nix`, path = `${ops.root}/${name}.json`, text = baselineExpr(source.source, phase);
    ...
    if (supplementalProtection(phase) && (!parsed.janette || !parsed.stibniteWorker)) throw new Stop("contract", "Missing supplemental protected projection");
    return { phase, source, path, digest: digest(value), expression, expressionDigest: digest(text), controller, command };
  });
}
export async function protectedComparison(ops: Operations, name: string, baseline: Projection, source: Source, requireCanonical = true) {
  if (source.role !== baseline.source.role) throw new Stop("contract", "Protected comparison crosses source roles");
  const candidate = await projection(ops, `${name}-candidate`, source, baseline.phase);
  return ops.tool(name, { baseline, source, candidate, requireCanonical }, async (signal) => {
    ...
    return { baseline, candidate, ...compareProtected(baseline.phase, old, current, requireCanonical) };
  });
}
export const preserved = (phase: Phase, result: ReturnType<typeof compareProtected>) => result.serverUnchanged && (phase !== "capabilities" || result.humansUnchanged) && result.janetteUnchanged && result.stibniteWorkerUnchanged;
async function contextPreserved(ops: Operations, name: string, phase: Phase, baseline: Projection, previous: Source, current: Source) {
  ...
  const result = await protectedComparison(ops, name, baseline, integrated(current), false);
...
export default workflow({
  ...
  run: async (ctx) => {
    ...
    try {
      ...
      const historicalBaseline = await projection(ops, "protected-baseline-chain", base);
      const historicalIntegratedBaseline = await projection(ops, "protected-baseline-integrated", integrated(base));
      const supplemental = supplementalProtection(spec.phase);
      const baseline = supplemental ? await projection(ops, "protected-baseline-supplemental-chain", base, spec.phase) : historicalBaseline;
      const integratedBaseline = supplemental ? await projection(ops, "protected-baseline-supplemental-integrated", integrated(base), spec.phase) : historicalIntegratedBaseline;
      const baselineEvidence = await ops.save("baselines", { baseline: historicalBaseline, integratedBaseline: historicalIntegratedBaseline, baselineOwned, supplementalBaseline: supplemental ? baseline : null, supplementalIntegratedBaseline: supplemental ? integratedBaseline : null });
      const baselineReads = [...new Set([historicalBaseline.path, historicalIntegratedBaseline.path, baseline.path, baseline.expression, integratedBaseline.path, integratedBaseline.expression])];
      for (let attempt = 0; attempt <= input.max_repairs; attempt++) {
        ...
        if (!input.verify_only) {
          const report = await ops.stage(`implement-${suffix}`, {
            ...
            reads: [research, baselineEvidence, ...baselineReads, ...input.reads, ...feedback], output: `${input.root}/implement-${suffix}.md`, outputMode: "file-only",
        ...
        const report = await ops.stage(`review-${suffix}`, {
          ...
          reads: [research, evidence, diff, baselineEvidence, ...baselineReads, ...input.reads], output: reviewPath, outputMode: "file-only",
